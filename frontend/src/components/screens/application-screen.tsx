        ...
        action={
          onRun ? (
            <Button type="button" onClick={onRun} disabled={running}>
              {running ? "Running screen…" : "Run screen"}
            </Button>
          ) : undefined
        }
      >
    ...
    return (
      <PendingPanel
        action={
          onRun ? (
            <Button type="button" onClick={onRun} disabled={running}>
              {running ? "Drafting memo…" : "Draft memo"}
            </Button>
          ) : undefined
        }
      >
        The memo has not been drafted yet.
      </PendingPanel>
    );
  }
    ...
    return (
      <PendingPanel
        action={
          memoReady && onRun ? (
            <Button type="button" onClick={onRun} disabled={running}>
              {running ? "Running adversary…" : "Run devil’s advocate"}
            </Button>
          ) : undefined
        }
      >
        {memoReady
          ? "Devil’s advocate has not run yet."
          : "Draft the memo first, then run devil’s advocate."}
      </PendingPanel>
    );
  }
  return (
...
type PipelineAction = "screen" | "diligence" | "memo" | "adversary";
...
export function ApplicationScreen({
  ...
  const [running, setRunning] = React.useState<PipelineAction | null>(null);
  const [error, setError] = React.useState<string | null>(null);

  React.useEffect(() => {
    setActive(snapshots[snapshots.length - 1] ?? null);
  }, [snapshots]);
  ...
  async function refresh() {
    const next = await getApplicationSnapshots(app.application_id);
    setActive(next[next.length - 1] ?? null);
  }

  async function runStep(action: PipelineAction, fn: () => Promise<unknown>) {
    setRunning(action);
    setError(null);
    try {
      await fn();
  ...
  const stages: StageDef[] = [
    ...
    },
    {
      ...
      done: app.adversarial !== null,
      chip:
        app.adversarial !== null
          ? `${app.adversarial.objections.length} objection${
  ...
  const [claimsStage, screeningStage, diligenceStage, memoStage, adversarialStage] =
    stages;
        ...
        <div className="space-y-6">
            ...
            <ScreeningStage
              ...
              onRun={
                app.axes === null
                  ? () =>
                      runStep("screen", () =>
                        screenApplication(app.application_id),
                      )
                  : undefined
              }
            />
          ...
          <StageSection stage={diligenceStage}>
            <DiligenceStage
              ...
              running={running === "diligence"}
              onRun={
                app.axes !== null && app.diligence === null
                  ? () =>
                      runStep("diligence", () =>
                        runDiligence(app.application_id),
                      )
                  : undefined
              }
            />
          ...
          <StageSection stage={memoStage}>
            <MemoStage
              ...
              diligence={app.diligence}
              evidence={app.evidence}
              running={running === "memo"}
              onRun={
                app.diligence !== null && app.memo === null
                  ? () =>
                      runStep("memo", () => draftMemo(app.application_id))
                  : undefined
              }
            />
          </StageSection>
          ...
          >
            <AdversarialStage
              ...
              running={running === "adversary"}
              onRun={
                app.memo !== null && app.adversarial === null
                  ? () =>
                      runStep("adversary", () =>
                        runAdversary(app.application_id),
                      )
                  : undefined
              }
            />
