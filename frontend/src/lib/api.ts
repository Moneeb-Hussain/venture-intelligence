...
export function getScanStatus(): Promise<ScanStatusEntry[]> {
  return getJson("/api/scan/status", fx.scanStatusFixture);
}

export function scanGithub(body?: {
  topics?: string[];
  since_days?: number;
}): Promise<PerSourceScanResponse> {
  return postJson(
    "/api/scan/github",
    body ?? { topics: ["gpu", "compiler", "mlops"], since_days: 14 },
    fx.scanGithubFixture,
  );
}

export function scanHn(body?: {
  query?: string;
  since_days?: number;
}): Promise<PerSourceScanResponse> {
  return postJson(
...
export async function scanSource(
  ...
  if (source === "github") return scanGithub();
  if (source === "hn") return scanHn();
  return scanYc();
}
